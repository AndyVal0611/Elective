...
namespace Elective
{
    internal class pos_dbconnection
    {
        ...
        public void pos_connString()
        {
            ...
            pos_connectionString = "Data Source=192.168.1.100,41414;Initial Catalog=POSDB;User ID=Andyval0612;Password=Andyval0612;";
        ...
        {
            // join on useraccountTbl.user_id because useraccountTbl has user_id (not emp_id)
            pos_sql = "SELECT p.emp_id, p.emp_fname, p.emp_surname, ua.pos_terminal_no, ua.account_type " +
                      "FROM pos_empRegTbl p INNER JOIN useraccountTbl ua ON p.emp_id = ua.user_id " +
                      "WHERE ua.account_type = 'Administrator'";
        }
